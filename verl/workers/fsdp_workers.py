...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_config.mode == "sync" and self._is_actor:
            # Python 3.12 removed the implicit event-loop creation of
            # asyncio.get_event_loop(): in a Ray worker thread with no loop set
            # (esp. under uvloop) it raises "There is no current event loop".
